...
categorize_cells <- function(data_sex, cell_types_clean_f) {
  data_cat <- data_sex[, c("OS_MONTHS", "OS_event", "AGE", cell_types_clean_f)]
...
data_female <- categorize_cells(
  data_wide[data_wide$SEX == "Female", ],
  cell_types_clean_f
)
...
plot_df_f$variable <- gsub("high$", " (high)", plot_df_f$variable)

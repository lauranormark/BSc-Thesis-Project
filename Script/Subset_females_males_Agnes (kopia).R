...
clin <- read.delim("Data/gbm_tcga_gdc/data_clinical_patient.txt",
                   header = TRUE,
                   comment.char = "#",
...
mrna2 <- mrna[, c("Gene_Symbol", expr_key2$SAMPLE_ID)]
head(mrna2)

# Omformaterar nyckeln till metadata och se till att Pateint ID matchar ordningen 
sample_meta <- expr_key2[, c("SAMPLE_ID", "PATIENT_ID", "SEX")]
all(colnames(mrna2)[-1] == sample_meta$SAMPLE_ID) # allt matchar!!!
View(sample_meta)



# Delar upp males och females i metadatan kopplat till ID
female_samples <- sample_meta$SAMPLE_ID[sample_meta$SEX == "Female"]
male_samples   <- sample_meta$SAMPLE_ID[sample_meta$SEX == "Male"]

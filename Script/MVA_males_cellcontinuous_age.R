...
data_immunoabundance <- read.delim(
  "Filis/merged_immunoabundance_clinical.txt",
  header = TRUE,
  check.names = FALSE
)
...
cell_types_f <- c("B_cells", "Plasma_cells")
...
formula_mva <- as.formula(
  paste("Surv(OS_MONTHS, OS_event) ~ age_group +", #age_group +
...
forest_female <- ggplot(plot_df_f, aes(x = HR, y = variable)) +
  ...
  theme_classic() +
  labs(
    title = "Multivariate Cox — Males (age adjusted), cell types as continuous",

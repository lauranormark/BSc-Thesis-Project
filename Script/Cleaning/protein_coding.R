...
#kontrollera att alla gener är unika
any(duplicated(rna_protein_coding$GENENAME))

#fick output TRUE --> fixa
rna_protein_coding <- rna_protein_coding[
  !duplicated(rna_protein_coding$GENENAME),
]


#kontrollera att alla patienter är unika
any(duplicated(colnames(rna_protein_coding))) #får output FALSE, då är allt redan ok

#skapa ny fil som nu är clean med rätt genenamn, endast proteinkodande gener och inga dubletter
write.table(rna_protein_coding,
            file = "mrnadata_protein_coding_unique.txt",
            sep = "\t",
            quote = FALSE,
            row.names = FALSE)

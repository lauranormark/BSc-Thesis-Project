...
clinical_unique <- data_immunoabundance %>%
  select(PATIENT_ID, AGE, SEX, OS_STATUS, OS_MONTHS, DFS_STATUS, DFS_MONTHS) %>%
...
target_cell <- "B_cells"  # <-- Ändra till önskad celltyp
...
# Dela in i Hög/Låg baserat på medianen
median_abund <- median(clinical_cell$ABUNDANCE, na.rm = TRUE)
clinical_cell$ABUND_GROUP <- ifelse(
  clinical_cell$ABUNDANCE >= median_abund, "High", "Low"
)

table(clinical_cell$ABUND_GROUP)  # Checka att grupperna ser rimliga ut

fit_os_cell <- survfit(
  Surv(OS_MONTHS, OS_event) ~ ABUND_GROUP,
  data = clinical_cell
)
...
survdiff(Surv(OS_MONTHS, OS_event) ~ ABUND_GROUP, data = clinical_cell)
